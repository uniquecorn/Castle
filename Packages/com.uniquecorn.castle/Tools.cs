...
namespace Castle
{
    public static class Tools
    {
        ...
        public static int[] RandomNumEnumerable(int length)
        {
            return RandomNumEnumerable(length, new System.Random());
        }
        public static int[] RandomNumEnumerable(int length, System.Random rng)
        {
            ...
            while (length > 1)
            {
                ...
                (range[k], range[length]) = (range[length], range[k]);
            }
            return range;
        }
        ...
        public static bool Intersect(Vector2 A, Vector2 B, Vector2 C, Vector2 D,out Vector2 intersection,out float alpha)
        {
            var a = ((D.x - C.x) * (C.y - A.y)) - ((D.y - C.y) * (C.x - A.x));
            var b = ((D.x - C.x) * (B.y - A.y)) - ((D.y - C.y) * (B.x - A.x));
            var c = ((B.x - A.x) * (C.y - A.y)) - ((B.y - A.y) * (C.x - A.x));

            alpha = a/b;
            var beta = c / b;
            if (Mathf.Abs(b) <= 0.0001f || alpha < 0 ||alpha > 1 || beta < 0 || beta > 1)
            {
